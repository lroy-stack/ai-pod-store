...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const titles: Record<string, Record<string, string>> = {
    en: { title: 'Privacy Policy', description: 'How ' + (process.env.NEXT_PUBLIC_SITE_NAME || 'My POD Store') + ' collects, uses, and protects your personal data' },
    es: { title: 'Política de Privacidad', description: 'Cómo ' + (process.env.NEXT_PUBLIC_SITE_NAME || 'My POD Store') + ' recopila, usa y protege tus datos personales' },
    de: { title: 'Datenschutzrichtlinie', description: 'Wie ' + (process.env.NEXT_PUBLIC_SITE_NAME || 'My POD Store') + ' Ihre personenbezogenen Daten erhebt, nutzt und schützt' },
  }
...
async function getLegalPage(slug: string): Promise<LegalPage | null> {
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      console.error('[privacy-page] Missing Supabase environment variables')
      return null
    }
    ...
    if (error) {
      console.warn('[privacy-page] Failed to fetch legal page:', error.message)
      return null
    }

    return data as LegalPage
  } catch (error) {
    console.error('[privacy-page] Error fetching legal page:', error)
    return null
  }
}
      ...
      <Card className="mt-8 border-primary/20 bg-primary/5">
        <CardContent className="py-6">
          <p className="text-sm text-muted-foreground text-center">
            {locale === 'en' && 'For questions about this policy, please contact our Data Protection Officer.'}
            {locale === 'es' && 'Para preguntas sobre esta política, contacte a nuestro oficial de protección de datos.'}
            {locale === 'de' && 'Bei Fragen zu dieser Richtlinie wenden Sie sich bitte an unseren Datenschutzbeauftragten.'}
          </p>
        </CardContent>
      </Card>
    </div>
