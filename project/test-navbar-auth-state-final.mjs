...
(async () => {
  ...
  try {
    ...
    await page.waitForTimeout(3000); // Extra wait for client-side state update
    
    // Reload the page to ensure navbar state is updated
    console.log('   Login successful, reloading page to verify navbar state...');
    ...
    console.log('7. Taking screenshot of logged-in navbar (after reload)...');
    await page.screenshot({ path: 'navbar-logged-in-en-reloaded.png', fullPage: true });
    console.log('   Screenshot saved: navbar-logged-in-en-reloaded.png');
    ...
    // Check for Cart and Orders links (only visible when logged in)
    const cartLink = await page.locator('nav a[href*="/cart"]').first();
    const cartVisible = await cartLink.isVisible().catch(() => false);
    console.log(`   Cart link visible: ${cartVisible}`);
    
    const ordersLink = await page.locator('nav a[href*="/orders"]').first();
    const ordersVisible = await ordersLink.isVisible().catch(() => false);
    console.log(`   Orders link visible: ${ordersVisible}`);
    ...
    if (!logoutVisible || !avatarVisible || !userNameVisible) {
      console.error('\n   ERROR: Logged-in state not complete after reload!');
    ...
    console.log('Step 3 - Logged-in state (EN after reload):');
    ...
    console.log('  - Cart link visible: ' + (cartVisible ? 'PASS' : 'FAIL'));
    console.log('  - Orders link visible: ' + (ordersVisible ? 'PASS' : 'FAIL'));
