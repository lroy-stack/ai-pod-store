...
test.describe('@e2e Chat Flow (Simple)', () => {
  test('Chat flow: send message and receive response', async ({ page }) => {
    ...
    // Find chat input - use multiple selectors as fallback
    let chatInput = page.locator('textarea[placeholder*="Ask"], input[placeholder*="Ask"]').first()

    // If not found, try simpler selectors
