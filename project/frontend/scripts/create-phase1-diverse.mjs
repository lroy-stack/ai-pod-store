/**
 * SKAPARA — Phase 1: 20 Diverse Products (6 types, 3 techniques)
 *
 * Reads catalog.json + SVGs from phase1-production/
 * Renders SVGs → PNG via sharp, uploads to Printify, creates products.
 *
 * Usage:
 *   node scripts/create-phase1-diverse.mjs --preview     # Render PNGs only
 *   node scripts/create-phase1-diverse.mjs --dry-run     # Show what would be created
 *   node scripts/create-phase1-diverse.mjs               # Create on Printify + Supabase
 *   node scripts/create-phase1-diverse.mjs --only T01    # Create single product by ID
 *   node scripts/create-phase1-diverse.mjs --skip T01,T02 # Skip specific products
 */
...
const PHASE_DIR = join(ROOT, 'public', 'phase1-production')
...
// ─── Build print_areas with multi-position placeholders ──────────────────────
  ...
  for (const position of product.positions) {
    ...
    switch (position) {
      case 'front':
        ...
        break
      case 'back':
        // Wordmark centered near top (neckline area)
        img.x = 0.5; img.y = 0.15; img.scale = 0.35; img.angle = 0
        break
      case 'neck_outer':
        // S mark filling most of the label area
        img.x = 0.5; img.y = 0.5; img.scale = 0.8; img.angle = 0
        break
      case 'left_sleeve':
        // Small S mark on upper sleeve
        img.x = 0.5; img.y = 0.3; img.scale = 0.35; img.angle = 0
        break
      case 'back_hat':
      case 'left_hat':
      case 'right_hat':
        // S mark centered on hat position
        img.x = 0.5; img.y = 0.5; img.scale = 0.8; img.angle = 0
        break
      default:
        img.x = 0.5; img.y = 0.5; img.scale = 1; img.angle = 0
...
async function main() {
  ...
  console.log('  SKAPARA — Phase 1: 20 Diverse Products')
  ...
  if (!PREVIEW && !DRY_RUN) {
    ...
    // Wordmark (back position for garments)
    const wordmarkPath = join(PHASE_DIR, catalog.brandingAssets.wordmark)
    const wordmarkBuf = readFileSync(wordmarkPath)
    await delay(2000)
    const wmUp = await uploadImage('skapara-wordmark-white.png', wordmarkBuf)
    brandingUploads.wordmark = wmUp.id
    console.log(`    Wordmark: ${wmUp.id}`)

    // Neck label (neck_outer for T-shirts)
    const neckPath = join(PHASE_DIR, catalog.brandingAssets.neckLabel)
    const neckBuf = readFileSync(neckPath)
    await delay(2000)
    const nkUp = await uploadImage('skapara-neck-label-white.png', neckBuf)
    brandingUploads.neckLabel = nkUp.id
    console.log(`    Neck label: ${nkUp.id}`)

    // Cap S mark (render SVG → PNG first)
    const capSvgPath = join(PHASE_DIR, catalog.brandingAssets.capSmark)
    const capPng = await renderSvg(capSvgPath, 600, 300)
    await delay(2000)
    const capUp = await uploadImage('skapara-cap-smark-teal.png', capPng)
    brandingUploads.capSmark = capUp.id
    console.log(`    Cap S mark: ${capUp.id}`)
  ...
  for (const [idx, product] of products.entries()) {
    ...
    try {
      ...
      // Dark background preview (1/4 size)
      ...
      const designUp = await uploadImage(
        `phase1-${product.id}-${product.name.replace(/\s+/g, '-').toLowerCase()}.png`,
        designPng
      )
      ...
      // Branding positions
      if (product.positions.includes('back')) {
        if (product.variantSet.startsWith('BP1744')) {
          // Cap back = S mark
          uploadIds.back_hat = brandingUploads.capSmark
        } else {
          // Garment back = wordmark
          uploadIds.back = brandingUploads.wordmark
        }
      }
      if (product.positions.includes('neck_outer')) {
        uploadIds.neck_outer = brandingUploads.neckLabel
      }
      if (product.positions.includes('left_sleeve')) {
        uploadIds.left_sleeve = brandingUploads.neckLabel // same asset
      }
      if (product.positions.includes('back_hat')) {
        uploadIds.back_hat = brandingUploads.capSmark
      }
      if (product.positions.includes('left_hat')) {
        uploadIds.left_hat = brandingUploads.capSmark
      }
      if (product.positions.includes('right_hat')) {
        uploadIds.right_hat = brandingUploads.capSmark
      }
      ...
      try {
        await delay(1500)
        // Try the dedicated GPSR endpoint
      ...
      const productDetails = {
        ...
        manufacturing_country: product.gpsr === 'embroidery' ? 'Latvia' : 'Germany',
        brand: 'SKAPARA',
        model: vs.blueprint_id === 6 ? 'Gildan 5000' :
               vs.blueprint_id === 77 ? 'Gildan 18500' :
               vs.blueprint_id === 49 ? 'Gildan 18000' :
               vs.blueprint_id === 80 ? 'Gildan 2400' :
               vs.blueprint_id === 1018 ? 'Two-Tone Mug' :
               vs.blueprint_id === 1744 ? 'Yupoong 6089M' : '',
      }
