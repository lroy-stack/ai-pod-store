/**
 * Generate SKAPARA branded designs at CORRECT dimensions per product type.
 * Each product type gets designs sized to its Printify placeholder.
 * Zero cost — SVG + Sharp.
 */
...
// ─── SVG Builders per product type ──────────────────────────────────────────

// S mark original viewBox: 0 0 14310 11000 (units in 0.1)
// Wordmark original viewBox: 0 0 20400 2316 (units in 0.1)

function buildMarkPaths(fill) {
  return S_MARK_PATHS.map(d => `<path d="${d}"/>`).join('\n        ')
}

function buildWordmarkPaths(fill) {
  return WORDMARK_PATHS.map(d => `<path d="${d}"/>`).join('\n        ')
}

/**
 * VERTICAL layout: S mark on top, wordmark below.
 * Good for: T-shirts (3951x4919), Hoodies (1200x1200)
 */
function buildVerticalSVG(w, h, fillColor, gradientDef = '') {
  const fill = gradientDef ? 'url(#brand-grad)' : fillColor

  // S mark: viewBox ~14310 wide x ~11000 tall → aspect ~1.3:1
  // Wordmark: viewBox ~20400 wide x ~2316 tall → aspect ~8.8:1
  // Layout: mark takes upper 65%, wordmark takes lower 15%, padding 10% each side

  const padX = w * 0.12
  const contentW = w - padX * 2
  const padTop = h * 0.10

  // S mark: fit within content area, upper portion
  const markNatW = 14310
  const markNatH = 11000
  const markMaxH = h * 0.55
  const markScale = Math.min(contentW / markNatW, markMaxH / markNatH)
  const markW = markNatW * markScale
  const markH = markNatH * markScale
  const markX = (w - markW) / 2
  const markY = padTop

  // Wordmark: below mark, centered
  const wmNatW = 20400
  const wmNatH = 2316
  const wmMaxW = contentW * 0.9
  const wmScale = wmMaxW / wmNatW
  const wmW = wmNatW * wmScale
  const wmH = wmNatH * wmScale
  const wmX = (w - wmW) / 2
  const wmY = markY + markH + h * 0.04
...
/**
 * HORIZONTAL layout: S mark on left, wordmark on right.
 * Good for: Bottles/Tumblers (wide & short)
 */
function buildHorizontalSVG(w, h, fillColor, gradientDef = '') {
  const fill = gradientDef ? 'url(#brand-grad)' : fillColor
  ...
  // S mark: fit in left ~35% of width
  const markNatW = 14310
  const markNatH = 11000
  const markMaxW = w * 0.30
  const markMaxH = contentH * 0.85
  const markScale = Math.min(markMaxW / markNatW, markMaxH / markNatH)
  const markW = markNatW * markScale
  const markH = markNatH * markScale
  const markX = padX + (markMaxW - markW) / 2
  const markY = padY + (contentH - markH) / 2

  // Wordmark: fit in right ~60% of width, vertically centered
  const wmNatW = 20400
  const wmNatH = 2316
  const wmMaxW = w * 0.58
  const wmScale = wmMaxW / wmNatW
  const wmW = wmNatW * wmScale
  const wmH = wmNatH * wmScale
  const wmX = padX + markMaxW + w * 0.04
  const wmY = (h - wmH) / 2
...
const GRADIENT_DEF = `
    <linearGradient id="brand-grad" x1="0.7" y1="0" x2="0.2" y2="1">
      <stop offset="0%" stop-color="#F97066"/>
      <stop offset="18%" stop-color="#E22C8B"/>
      <stop offset="35%" stop-color="#C42E86"/>
      <stop offset="50%" stop-color="#9C5BD6"/>
      <stop offset="65%" stop-color="#5438CD"/>
      <stop offset="80%" stop-color="#2536A4"/>
      <stop offset="100%" stop-color="#40ACCC"/>
    </linearGradient>`

const WARM_GRADIENT_DEF = `
    <linearGradient id="brand-grad" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="#FF6B35"/>
      <stop offset="33%" stop-color="#F7258A"/>
      <stop offset="66%" stop-color="#9C27B0"/>
      <stop offset="100%" stop-color="#651FFF"/>
    </linearGradient>`

const OCEAN_GRADIENT_DEF = `
    <linearGradient id="brand-grad" x1="0" y1="0" x2="1" y2="1">
...
// Printify placeholder dimensions:
//   T-shirt BP 6:  front 3951x4919
//   Hoodie BP 793: front_center_chest 1200x1200
//   Bottle BP 482: front 2100x1200
//   Bottle BP 854: front 2759x1500
//   Tumbler BP 353: front 2795x2100
//   Tumbler BP 693: front 2900x1181

const designs = [
  // T-shirt designs (vertical, 3951x4919)
  { name: 'tee-white',    svg: buildVerticalSVG(3951, 4919, '#FFFFFF') },
  { name: 'tee-dark',     svg: buildVerticalSVG(3951, 4919, '#0F172A') },
  { name: 'tee-gradient', svg: buildVerticalSVG(3951, 4919, null, GRADIENT_DEF) },

  // Hoodie designs (vertical, 1200x1200 — chest area)
  { name: 'hoodie-white', svg: buildVerticalSVG(1200, 1200, '#FFFFFF') },
  { name: 'hoodie-dark',  svg: buildVerticalSVG(1200, 1200, '#0F172A') },

  // Bottle 482 (horizontal, 2100x1200)
  { name: 'bottle-gradient', svg: buildHorizontalSVG(2100, 1200, null, GRADIENT_DEF) },

  // Bottle 854 (horizontal, 2759x1500)
  { name: 'bottle-dark', svg: buildHorizontalSVG(2759, 1500, '#0F172A') },

  // Tumbler 353 (near-square, 2795x2100)
  { name: 'tumbler-warm', svg: buildVerticalSVG(2795, 2100, null, WARM_GRADIENT_DEF) },

  // Tumbler 693 (very horizontal, 2900x1181)
  { name: 'tumbler-ocean', svg: buildHorizontalSVG(2900, 1181, null, OCEAN_GRADIENT_DEF) },
]
...
async function renderDesigns() {
  ...
  console.log(`\nAll brand designs saved to: ${OUT_DIR}`)
}
