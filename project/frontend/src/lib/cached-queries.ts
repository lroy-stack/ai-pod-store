...
export async function setCachedProductDetail(id: string, product: any) {
  await setCached(`product:${id}`, product, TTL.PRODUCT_DETAIL)
}
...
export async function invalidateProductCache(productId?: string) {
  ...
  // Clear specific product if provided
  if (productId) {
