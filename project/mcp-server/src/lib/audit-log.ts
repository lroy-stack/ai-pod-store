...
export function withAuditLog<TInput, TExtra extends { authInfo?: AuthInfo } | undefined>(
  ...
  return async (input: TInput, extra?: TExtra) => {
    ...
    try {
      ...
      // Extract userId from authInfo.extra (where it's injected by our auth system)
      const userId = extra && 'authInfo' in extra && extra.authInfo?.extra?.userId
        ? String(extra.authInfo.extra.userId)
        : undefined;
