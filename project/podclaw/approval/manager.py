...
class ApprovalManager:
    ...
    async def request_approval(
        ...
        # 2. Send preview with image if available
        if preview_image_url:
            await self._responder.send_to_ceo(
                platform, preview_text, image_url=preview_image_url
            )
        ...
        # Use WhatsApp/Telegram buttons via responder's connector
        await self._send_buttons(platform, preview_text if not preview_image_url else "Que opinas?", buttons)
