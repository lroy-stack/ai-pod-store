...
_PATTERNS: list[tuple[re.Pattern, EventType]] = [
    ...
    (re.compile(r"(publica|publish|catálogo|catalog|lista\w*\s+producto|actualiza\w*\s+producto|borra\w*\s+producto)", re.I), EventType.CATALOG_REQUEST),
    (re.compile(r"(cuánto|vendimos|revenue|ingres|cost|gananc|pedido|orden|venta|factur|métric)", re.I), EventType.QUERY),
    (re.compile(r"(investiga|research|tendencia|trend|competencia|mercado|nicho)", re.I), EventType.RESEARCH_REQUEST),
    (re.compile(r"(marketing|campaña|campaign|newsletter|promo)", re.I), EventType.MARKETING_REQUEST),
]
    ...
    async def _classify_with_haiku(self, text: str) -> EventType:
        ...
        try:
            ...
            response = client.messages.create(
                ...
                messages=[{
                    "role": "user",
                    "content": _HAIKU_PROMPT.format(text=text[:500]),
                }],
            )
