...
async def _notify_email(level: EscalationLevel, message: str, context: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        endpoint = f"{api_url}/api/internal/send-escalation-email"
        ...
        logger.info('[Escalation] Email notification sent')
        return True
...
async def _record_escalation(level: EscalationLevel, message: str, context: Dict[str, Any]) -> None:
    """Record escalation in database for audit trail"""
    try:
        # Call frontend API to record escalation
        api_url = os.getenv('FRONTEND_URL', 'http://localhost:3000')
        endpoint = f"{api_url}/api/internal/record-escalation"
