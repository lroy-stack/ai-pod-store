...
2. **Determine the design style** → Check DESIGN_GUIDELINES.md for SKAPARA brand rules
...
### Neck Position (`neck_outer`) — SKAPARA Branding

Products offering `neck_outer` (BP6, BP12, BP145) **should use it for SKAPARA branding**:
- **Dark garments** (Black, Navy, Dark Heather): Use `skapara-mark-white.svg` (white S mark)
- **Light garments** (White, Sport Grey): Use `skapara-mark-dark.svg` (navy #0F172A S mark)
- **Kids products**: Use `skapara-mark-color.svg` (gradient S mark — more colorful, fun)
...
- **Option A: Wordmark only** — "SKAPARA" text centered, small (~15-20% of canvas), `y: 0.15` (upper back between shoulders)
...
4. **Kids products** prefer the colorful gradient mark (`skapara-mark-color.svg`)
...
**Recommended garment colors for SKAPARA** (dark-first brand):
